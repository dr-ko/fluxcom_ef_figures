...
dats={
'rs':[exDrive+'FLUXCOM_Eflux/analysis_eFlux_paper_iter4_201903/data.local/tEnergyFluxes/RS/ensembles/common/Global/Rn.RS.EBC-NONE.MLM-ALL.METEO-NONE.720_360.monthly.2001-2013.nc','Rn','RS',1],\
'rsm':[exDrive+'FLUXCOM_Eflux/analysis_eFlux_paper_iter4_201903/data.local/tEnergyFluxes/RS_METEO/ensembles/common/Global/Rn.RS_METEO.EBC-NONE.MLM-ALL.METEO-ALL.720_360.monthly.2001-2013.nc','Rn','RS+METEO',1],\
'crs':[exDrive+'FLUXCOM_Eflux/data/obs/Rn_CERES/Rn.CERES.Ed4A.720.360.monthly.2001-2010.nc','Rn','CERES (Ed4A)',0.0864],\
'srb':[exDrive+'FLUXCOM_Eflux/data/obs/Rn_SRB/NETrad.SRB.rel3_1.720.360.monthly.2001-2007.nc','NETrad','SRB (Rel. 3.1)',1],\

}
models='rs rsm crs srb'.split()
...
for row_m in range(nmodels):
    ...
    for col_m in range(nmodels):
        ...
        if row_m == col_m:
            ...
            gl_mean=np.nansum(np.ma.masked_equal(mod_dat_row,fill_val).filled(0)*ar_w)/np.nansum(ar_w)
            ...
            tit_str="$\\mu_{Rn}$ = "+str(round(gl_mean,2))+' ('+varibs_info[plotVar][1]+')'
...
mtool.mk_colo_cont(_axcol_rat,_bounds_rat,cm_rat,cbfs=0.9*ax_fs,cbrt=90,cbtitle=cb_tit)
plt.savefig('Figure_05_r1.png',bbox_inches='tight',bbox_extra_artists=[t_x],dpi=450)
plt.savefig('Figure_05_r1.pdf',bbox_inches='tight',bbox_extra_artists=[t_x],dpi=450)
plt.show()
